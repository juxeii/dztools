...
public class BrokerStopTest extends CommonUtilForTest {
    ...
    public class OnValidOrderForSetSL {
        ...
        private void setParamsRunnerResult(final OrderActionResult result) {
            when(taskParamsRunnerMock.startSetSL(orderMock, brokerStopData))
                .thenReturn(result);
        }

        @Test
        public void setSLFailsWhenParamsRunnerReturnsFail() {
            setParamsRunnerResult(OrderActionResult.FAIL);

            assertBrokerStopResult(ZorroReturnValues.ADJUST_SL_FAIL);
        }

        @Test
        public void setSLIsOKWhenParamsRunnerReturnsIsOK() {
            setParamsRunnerResult(OrderActionResult.OK);
